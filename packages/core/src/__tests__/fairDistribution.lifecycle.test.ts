...
describe('fair distribution - instance lifecycle churn', () => {
  it(
    ...
    async () => {
      ...
      await runRapidChurnIterations(backend, activeLimiters, ITERATIONS);
...
describe('fair distribution - instance lifecycle rejoin', () => {
  it(
...
describe('fair distribution - availability callback', () => {
  it(
    'emits allocation updates when slots change',
    async () => {
      const backend = new FairDistributionBackend({ totalCapacity: HUNDRED, estimatedTokensPerRequest: TEN });

      const slotsHistory: number[] = [];
      const onSlotsChange = (slots: number): void => {
        slotsHistory.push(slots);
      };
