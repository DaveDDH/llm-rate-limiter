...
const assertLimitsNeverExceeded = (
...
const assertJobAccountingCorrect = (
  tracker: JobTracker,
  totalJobs: number,
  stats: ReturnType<DistributedBackendInstance['getStats']>
): void => {
...
describe('extreme load - simultaneous burst from many instances', () => {
  it(
    'should NEVER exceed limits when 10 instances fire 100 jobs each simultaneously',
    async () => {
      const TPM = THOUSAND;
      const RPM = HUNDRED;
      const INST = TEN;
      ...
      const instances = createConnectedLimiters(INST, backend, (b) =>
        createLLMRateLimiter({ backend: b, models: { default: createModelConfig(TPJ) } })
      );
      const tracker = createJobTracker();
      await fireSimultaneousJobs(
        instances,
        JPI,
        { getTokens: () => TPJ, getDelay: () => randomInt(ONE, TEN) },
        tracker
      );
  ...
  it(
    ...
    async () => {
      ...
      const backend = createDistributedBackend({
        tokensPerMinute: TPM,
        requestsPerMinute: RPM,
        estimatedTokensPerRequest: AVG,
      });
      const instances = createConnectedLimiters(INST, backend, (b) =>
        createLLMRateLimiter({ backend: b, models: { default: createModelConfig(AVG) } })
      );
      const tracker = createJobTracker();
      await fireSimultaneousJobs(
        instances,
...
describe('extreme load - 2000 concurrent jobs from 10 instances', () => {
  it(
    'should handle massive concurrent load without ever exceeding limits',
    async () => {
      ...
      await fireSimultaneousJobs(
        instances,
        JPI,
        { getTokens: () => TPJ, getDelay: () => randomInt(ONE, TWENTY) },
        tracker
      );
      const stats = backend.getStats();
      assertLimitsNeverExceeded(stats, TPM, RPM);
...
describe('extreme load - 5000 jobs stress test', () => {
  it(
    'should handle 5000 jobs across 10 instances without any limit violations',
    async () => {
      const TPM = FIVE_THOUSAND;
...
describe('extreme load - ultimate validation', () => {
  it(
    'THE ULTIMATE TEST: 10000 jobs, 20 instances, variable sizes, multiple windows',
    async () => {
      const TPM = THOUSAND;
      const RPM = HUNDRED;
      const INST = TWENTY;
