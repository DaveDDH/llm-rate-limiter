...
describe('distributed - token coordination', () => {
  it('should coordinate token usage across instances', async () => {
    ...
    if (instance1 === undefined || instance2 === undefined) {
      throw new Error('Instances not created');
    }
    await instance1.limiter.queueJob({
      jobId: 'job1',
      job: ({ modelId }, resolve) => {
        resolve({ modelId, inputTokens: TWENTY, cachedTokens: ZERO, outputTokens: ZERO });
        return { requestCount: ONE, usage: { input: TWENTY, output: ZERO, cached: ZERO } };
      },
    });
  ...
  it('should reject when combined usage exceeds limit', async () => {
    ...
    await instance1.limiter.queueJob({
      jobId: 'job1',
      job: ({ modelId }, resolve) => {
        resolve({ modelId, inputTokens: FIFTY, cachedTokens: ZERO, outputTokens: ZERO });
        return { requestCount: ONE, usage: { input: FIFTY, output: ZERO, cached: ZERO } };
      },
    });
