...
describe('availabilityTracker - all change reasons', () => {
  it('should detect tokensPerDay/requestsMinute/requestsDay changes', () => {
    ...
    let reqMin = HUNDRED;
    let reqDay = HUNDRED;
    const { tracker: t1, calls: c1 } = createChangeTracker(
      () => ({
        models: {
          default: {
    ...
    t1.initialize();
    tokensDay = FIFTY;
    t1.checkAndEmit('tokensDay');
    expect(c1.includes('tokensDay')).toBe(true);
    const { tracker: t2, calls: c2 } = createChangeTracker(
      () => ({
        models: {
          default: {
    ...
    t2.initialize();
    reqMin = FIFTY;
    t2.checkAndEmit('requestsMinute');
    expect(c2.includes('requestsMinute')).toBe(true);
    const { tracker: t3, calls: c3 } = createChangeTracker(
      () => ({
        models: {
          default: {
    ...
    t3.initialize();
    reqDay = FIFTY;
    t3.checkAndEmit('requestsDay');
    expect(c3.includes('requestsDay')).toBe(true);
  });
  it('should detect concurrentRequests/memory changes and skip equal', () => {
    ...
    t1.initialize();
    conc = FIFTY;
    t1.checkAndEmit('concurrentRequests');
    expect(c1.includes('concurrentRequests')).toBe(true);
    const { tracker: t2, calls: c2 } = createChangeTracker(
      () => ({
        models: { default: {} },
    ...
    t2.initialize();
    memKB = HUNDRED;
    t2.checkAndEmit('memory');
    expect(c2.includes('memory')).toBe(true);
    const { tracker: t3, calls: c3 } = createChangeTracker(
      () => ({
        models: {
          default: {
    ...
    t3.initialize();
    t3.checkAndEmit('tokensMinute');
    expect(c3.length).toBe(ZERO);
  });
});
